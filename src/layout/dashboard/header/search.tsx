...
import { useSearch } from "@/layout/store/use-search";
...
export function Search({ className = "", placeholder }: SearchProps) {
  ...
  return (
    <Button
      variant="outline"
      className={cn(
        "group bg-muted/25 text-muted-foreground hover:bg-accent relative mx-2 h-8 w-full flex-1 cursor-pointer justify-start rounded-md text-sm font-normal shadow-none sm:w-40 sm:pe-12 md:flex-none lg:w-52 xl:w-64",
        className
      )}
